...
<body>
    ...
    <div class="logo-container">
        <?php foreach ($brands as $brand): ?>
            <img src="<?php echo base_url('assets/image/logo/' . $brand->image) ?>" alt="<?php echo $brand->name ?>" class="logo" onclick="showAddresses(<?php echo $brand->id ?>)">
        <?php endforeach; ?>
    </div>
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    data.forEach(function(address) {
                        var logos = address.brand_images.split(',').map(function(image) {
                            return '<img src="<?php echo base_url('assets/image/logo/') ?>' + image + '" alt="Logo" class="logo">';
                        }).join('');
